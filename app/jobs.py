...
def _set_progress(job, stage: str, current: int, total: int, error: str | None = None) -> None:
    if not job:
        return
    job.meta.update(
        {
...
def process_transcription_job(
    ...
    _set_progress(current_job, "downloading", 0, 1)
    ...
    try:
        ...
        _set_progress(current_job, "callback", 0, 1)
        _post_callback(callback_url, done_payload)
        _set_progress(current_job, "done", 1, 1)
        logger.info("Job completed in %.2fs job_id=%s", time.time() - started, job_id)
        return done_payload

    except Exception as exc:
        _set_progress(current_job, "error", 0, 1, str(exc))
